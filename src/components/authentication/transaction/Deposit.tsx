...
export const Deposit = (props: DepositProps) => {
    ...
    const sendToken = async (amountInput: number): Promise<void> => {
        ...
        const requestBody: { address: string } = { address: sender }
        const responseBody: { status: true, token: string } | { status: false, error: string }
            = await (await fetch(endpoint + '/get-verify-token', {
        ...
        const sendResult: DeliverTxResponse = await client.sendTokens(sender, receiver, amount, fee, token);
